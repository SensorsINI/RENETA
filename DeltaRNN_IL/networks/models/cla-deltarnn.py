...
from networks.layers.deltalstm3_2 import DeltaLSTM3_2   # Gamma Δx/h
from networks.layers.deltalstm3_3 import DeltaLSTM3_3   # Learnable theta
...
class Model(nn.Module):
    def __init__(self, proj):
        ...
        elif self.rnn_type == 'LSTM3_2':
            proj.additem("rnn_layer", "DeltaLSTM3_2")
            self.rnn = DeltaLSTM3_2(input_size=self.input_size,
                                    hidden_size=self.rnn_size,
                                    num_layers=self.rnn_layers,
                                    thx=self.thx,
                                    thh=self.thh,
        ...
        elif self.rnn_type == 'LSTM3_3':
            proj.additem("rnn_layer", "DeltaLSTM3_3")
            self.rnn = DeltaLSTM3_3(input_size=self.input_size,
                                    hidden_size=self.rnn_size,
                                    num_layers=self.rnn_layers,
                                    thx=self.thx,
                                    thh=self.thh,
    ...
    def load_pretrain_model(self, path, il_mode):
        model_location = next(self.parameters()).device
        state_dict = torch.load(path, map_location=model_location)
        # Add gamma parameters for DeltaLSTM3_2
        if self.rnn_type == 'LSTM3_2':
            for i in range(self.rnn_layers):
                input_size = self.input_size if i == 0 else self.rnn_size
                state_dict[f"rnn.gamma_x.{i:d}"] = nn.Parameter(torch.ones(input_size))
                state_dict[f"rnn.gamma_h.{i:d}"] = nn.Parameter(torch.ones(self.rnn_size))
        # Add theta parameters for DeltaLSTM3_3
        if self.rnn_type == 'LSTM3_3':
            for i in range(self.rnn_layers):
                input_size = self.input_size if i == 0 else self.rnn_size
                state_dict[f"rnn.th_x.{i:d}"] = self.rnn.th_xh_init[i][:input_size]
                state_dict[f"rnn.th_h.{i:d}"] = self.rnn.th_xh_init[i][input_size:]
        # Extend FC layer parameters
        if il_mode:
            with torch.no_grad():
                for name, param in self.cl.named_parameters():
                    # print('From state_dict:', state_dict['cl.'+name])
                    # print(name, param)
                    cl_param = param.detach().clone()
                    cls_old = state_dict['cl.'+name].size()[0]
                    cl_param[:cls_old] = state_dict['cl.'+name]
                    state_dict['cl.'+name] = cl_param
                    # print('New param:', name, cl_param)
