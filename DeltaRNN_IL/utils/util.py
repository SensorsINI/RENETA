...
def length_to_mask(lengths, batch_first=True, max_len=None, dtype=None):
    """lengths: (B, ).
    return (B, max_len) if batch_first = True, else (max_len, B).
    If max_len is None, then max of lengths will be used.
    """
    assert len(lengths.shape) == 1, 'Input \'lengths\' shape should be 1 dimensional.'
