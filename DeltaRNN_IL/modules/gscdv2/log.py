...
def gen_paths(proj):
    ...
    # Pretrain Model Path
    if proj.il_mode == 0:
        step_folder = 'pretrain'
    else:
        if proj.il_load_pretrain:
            step_folder = 'pretrain'
        else:
            step_folder = 'retrain'
    pretrain_file = os.path.join('./save', proj.dataset, step_folder, pretrain_model_id + '.pt')
...
def gen_model_id(proj):
    # Custom String
    str_custom = '' if proj.custom_string == '' else f"{proj.custom_string}_"
    ...
    def gen_net_arch(model_name, num_classes):
        ...
        str_net_arch += '_C_' + f"{num_classes:d}"
    ...
    str_net_arch = gen_net_arch(proj.model_name, proj.num_classes)
    ...
    if proj.il_mode == 0:
        ...
        if proj.num_learned_classes == 0:
            ...
            num_classes = proj.num_learned_classes + 1
    str_net_arch_pretrain = gen_net_arch(proj.model_pretrain, num_classes)
    if 'delta' in proj.model_pretrain:
        str_net_arch_pretrain += '_TX_' + f"{proj.thx:.2f}" + '_TH_' + f"{proj.thh:.2f}"
    pretrain_model_id = str_custom + str_setting + str_net_arch_pretrain
    # Remove "Delta" from the model ID to load the non-delta network
    # if proj.il_mode == 0:
    #     pretrain_model_id = pretrain_model_id.replace("Delta", "")
    if proj.pretrain_file != '':
        pretrain_model_id = proj.pretrain_file
    ...
    model_id = str_custom + str_setting + str_net_arch
    ...
    if proj.save_every_epoch:  # Save every epoch
        ...
        if score_val:
            ...
            if epoch == 0 or dev_stat['LOSS'] < best_metric:
                ...
                best_epoch = epoch
                print('>>> saving best model from epoch %d to %s' % (epoch, proj.save_file))
    print(f"Best Metric: {best_metric:f}\n")
    return best_metric
