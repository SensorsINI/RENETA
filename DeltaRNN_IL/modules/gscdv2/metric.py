    ...
    def get_decisions(self, proj):
        for k, v in self.data.items():
            if k == 'outputs':
                max_nt = 0
                for vi in v:
                    max_nt = vi.shape[1] if vi.shape[1] > max_nt else max_nt
                for i, vi in enumerate(v):
                    if vi.shape[1] < max_nt:
                        pad_len = max_nt - vi.shape[1]
                        v[i] = np.pad(vi, ((0, 0), (0, pad_len), (0, 0)), 'constant', constant_values=0)
            self.data[k] = np.concatenate(v, axis=0)
        # y_true = np.amax(self.data['targets_metric'], axis=-1)
        # Greedy Decoder
        # self.data['outputs'].shape == (num_samples, max_seq_length, num_classes)
        idx_pred = np.argmax(self.data['outputs'], axis=-1)
        # idx_pred.shape == (num_samples, max_seq_length)
        score_pred = np.amax(self.data['outputs'], axis=-1)
        seq_len = idx_pred.shape[-1]
        y_pred = []
        for seq in idx_pred:
            # Non-silence timesteps
            nonzero_idx = np.nonzero(seq)[0]
            if nonzero_idx.size != 0:
                decision_idx = np.amax(nonzero_idx) # The last non-silence timestep
            else:
                decision_idx = seq_len-1
            y_pred.append(seq[decision_idx])        # The last non-silence class
