...
class DataPrepare:
    ...
    def augment_noise(self, list_snr):
        ...
        for i, file_path in enumerate(tqdm(selected_paths, desc='Noise-Train')):
            keyword, local_path, file_name, file_dir, _, _, _ = self.decompose_data_path(file_path)
            # Select random noise track
            noise_file = np.random.choice(all_noise_files)
            noise, sample_rate = snd.read(noise_file)
    ...
    def collect(self, augment_noise, target_snr):
        ...
        for file_path in tqdm(all_file_paths, desc='Remove useless paths'):
            keyword, local_path, file_name, file_dir, _, ori_file_name, _ = self.decompose_data_path(file_path)
            ...
            if keyword == '_background_noise_' or local_path in self.test_list or local_path in self.val_list:
                continue
            else:
                selected_paths.append(file_path)
        ...
        #     selected_unknown_paths = random.sample(all_unknown_paths,
        #                                            num_sample_per_unknown_keywords_train)
        ...
        for idx, file_path in enumerate(tqdm(selected_paths, desc='Collect Train')):
        ...
        for file_path in all_file_paths:
            ...
            if augment_noise:
                ...
                if augment_noise == 0 and 'SNR' in file_name:
                    ...
                    # Use test_list and val_list for test set
                    if (local_path in self.test_list) or (local_path in self.val_list):
                        selected_paths.append(file_path)
